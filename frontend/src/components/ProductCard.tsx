...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <div className="group bg-background-card hover:bg-background-card-hover border border-border hover:border-accent/40 rounded-custom p-3 flex flex-col justify-between transition-all duration-300 hover:shadow-glow relative h-full">
      ...
      <Link href={ROUTES.PRODUCT_DETAIL(product.slug)} className="block relative aspect-[3/4] rounded-custom overflow-hidden bg-background-card-hover mb-3">
        {/* Badges */}
        ...
        <img
          src={secondUrl}
          alt={product.name}
          className="absolute inset-0 w-full h-full object-cover transition-all duration-700 ease-out scale-100 group-hover:scale-105 opacity-0 group-hover:opacity-100"
          loading="lazy"
        />
        ...
        <div className="absolute inset-0 bg-black/45 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-3">
          <button
            ...
            className="p-2.5 bg-background-card border border-border text-foreground hover:bg-accent hover:border-accent hover:text-white rounded-full transition-all duration-200 transform translate-y-4 group-hover:translate-y-0"
            title="Xem nhanh"
          >
            <Eye size={18} />
          </button>
          <button
            ...
            className={`p-2.5 bg-background-card border border-border rounded-full transition-all duration-200 transform translate-y-4 group-hover:translate-y-0 ${
              favorited
      ...
      <div className="mt-3 flex items-center justify-between gap-2 border-t border-border/50 pt-2.5">
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-custom font-bold text-xs uppercase transition-all duration-200 ${
            isOutOfStock
              ? 'bg-background-card-hover border border-border text-foreground-muted cursor-not-allowed'
              : 'bg-background-card border border-border hover:border-accent hover:bg-accent text-foreground hover:text-white cursor-pointer'
          }`}
        >
